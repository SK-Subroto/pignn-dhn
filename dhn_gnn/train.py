...
def main():
    ...
    train_ts, test_ts = config.split_timesteps(n_ts, args.split_every)
    if args.n_train:
        # even stride through the train half -> keeps the whole weather window
        idx = np.linspace(0, len(train_ts) - 1, args.n_train).round().astype(int)
    ...
    samples = make_samples(ops, train_ts)
    model = DHNUnrolledSolver(ops, **model_kwargs).float()
    r0, _ = mean_final_residual(model, samples)
    ...
    save_checkpoint(
        model, model_kwargs, config.CHECKPOINT,
